...
# 4. Pre-crear carpetas críticas (Evita Error 500)
RUN mkdir -p var/cache var/logs public/legacy/cache

# 5. Configurar DocumentRoot a /public y permisos de Apache
ENV APACHE_DOCUMENT_ROOT /var/www/html/public
RUN sed -ri -e 's!/var/www/html!${APACHE_DOCUMENT_ROOT}!g' /etc/apache2/sites-available/*.conf \
    && sed -ri -e 's!/var/www/!${APACHE_DOCUMENT_ROOT}!g' /etc/apache2/apache2.conf /etc/apache2/conf-available/*.conf \
    && sed -i 's/AllowOverride None/AllowOverride All/g' /etc/apache2/apache2.conf

# 6. Aplicar permisos finales (Escapando el punto y coma correctamente)
RUN chown -R www-data:www-data /var/www/html \
    && find /var/www/html -type d -exec chmod 2775 {} \; \
    && find /var/www/html -type f -exec chmod 0664 {} \; \
    && chmod +x /var/www/html/bin/console || true

# 7. Silenciar avisos de PHP que rompen el instalador web
